...
// Sécurité (06/09/2026) : cet endpoint agissait sur une vraie campagne
// Google Ads (argent réel) en ne vérifiant QUE le draftId transmis, jamais
// l'identité de l'appelant — or le draftId n'est pas un secret (il apparaît
// en clair dans l'URL finale des annonces réellement diffusées). N'importe
// qui connaissant le draftId d'un artisan pouvait donc piloter sa campagne
// à sa place. Même vérification de jeton que api/bloquer-creneau.js.
...
export default async function handler(req, res) {
  ...
  const { draftId, token, message } = req.body || {};
  if (!draftId || !token) {
