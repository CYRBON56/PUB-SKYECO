...
export default async function handler(req, res) {
  ...
  // "metier" peut être un tableau (sélection multiple) ou une chaîne unique
  // (anciens sites) — on gère les deux, et on combine les mots-clés.
  const metiersListe = Array.isArray(metier) ? metier : (metier ? [metier] : []);
  const keywordsBruts = metiersListe.length
    ? [...new Set(metiersListe.flatMap(m => KEYWORDS_BY_METIER[m] || []))]
    : KEYWORDS_BY_METIER.autre;
  // Plafond à 8 mots-clés max par estimation, même si plusieurs métiers sont
  // cochés — chaque mot-clé consomme un appel (donc des crédits) OpenRush.
  const keywords = keywordsBruts.slice(0, 8);
