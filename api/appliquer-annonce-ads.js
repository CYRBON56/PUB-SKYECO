...
// Sécurité (06/09/2026) : voir api/bloquer-creneau.js — ce endpoint créait
// une vraie annonce Google Ads sur simple présentation d'un draftId (non
// secret).
...
export default async function handler(req, res) {
  ...
  if (!draftId || !token) {
    return res.status(400).json({ success: false, error: 'draftId et token requis.' });
  }
