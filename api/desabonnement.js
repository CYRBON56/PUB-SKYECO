...
// Même jeton, deux tables possibles selon la campagne d'origine — voir
// commentaire équivalent dans api/ouverture.js.
async function desabonner(table, headers, p, champNom) {
  const lecture = await fetch(`${process.env.SUPABASE_URL}/rest/v1/${table}?clic_token=eq.${encodeURIComponent(p)}&select=id,${champNom}`, { headers });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let nom = await desabonner('prospects_vitrine', headers, p, 'nom');
    if (nom === null) nom = await desabonner('prospects_paysagiste', headers, p, 'nom_entreprise');
    if (nom === null) return res.status(200).send(pageConfirmation({ succes: false }));
    return res.status(200).send(pageConfirmation({ succes: true, nom }));
  } catch (err) {
