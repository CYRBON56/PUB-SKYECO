...
// le CPC max réel de la campagne sur simple présentation d'un draftId (non
// secret).
...
export default async function handler(req, res) {
  ...
  const { draftId, token, plafondCpc } = req.body || {};
  const montant = parseFloat(plafondCpc);
  if (!draftId || !token || !montant || montant <= 0) {
    return res.status(400).json({ success: false, error: 'draftId, token et plafondCpc (montant en €, > 0) requis.' });
  }
