...
//
// Pilote (voir claude/skyeco-pro-brief-marketplace-leads.md) : uniquement
// métier "résine de sol / EPDM", département 56 (Morbihan) — les deux sont
// fixés côté serveur ici, jamais envoyés par le formulaire, pour éviter
// qu'un appel direct à cet endpoint n'injecte un métier/zone arbitraire tant
// qu'aucune autre page de capture n'existe.
//
...
const DEPARTEMENT_PILOTE = '56';
const PRIX_CTS_PILOTE = 3000; // 30,00 € TTC — décision du 16/09/2026
...
export default async function handler(req, res) {
  ...
  try {
    const insertResp = await fetch(`${process.env.SUPABASE_URL}/rest/v1/skyeco_pro_leads_marketplace`, {
      ...
      body: JSON.stringify([{
        metier: METIER_PILOTE,
        departement: DEPARTEMENT_PILOTE,
        zone_region: 'Bretagne',
    ...
    if (phoneE164) {
      await envoyerSMS(
        phoneE164,
        `${prenomAffiche}, merci pour votre demande de devis résine de sol (Skyeco). Un professionnel qualifié de notre réseau en Morbihan va revenir vers vous très prochainement.`
      );
    }
    ...
    const texteAdmin = `🎯 Nouveau lead marketplace Skyeco (résine/EPDM, ${commune || codePostal || 'Morbihan'}) — ${prenom || ''} ${nom || ''}, ${telephone || ''}. Disponible à la vente (30€) dans le dashboard des artisans éligibles.`;
